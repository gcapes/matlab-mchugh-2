    ...
    function x=subFn(a)
        ...
        switch chooseModel
            ...
            case 'DiDe_with_rise_time'
                ...
                signalModel = calculate_signalModelF(e, a, N, v, rootsS, del, DEL, Grad, gamma);
    ...
    function F=subFnPlot(a,b)
        ...
        switch chooseModel
            ...
            case 'DiDe_with_rise_time'
                ...
                N = 1; % Number of half oscillations (i.e. N = 1 for PGSE)
                v = N./(2.*del);
                F = calculate_signalModelF(e, a, N, v, rootsS, del, DEL, Grad, gamma);
...
function F = calculate_signalModelF(e, a, N, v, rootsS, del, DEL, Grad, gamma)
    %calculate_signalModelF   Calculates what is variously referred to in
    % the original code as 'signalModel' and 'F'.
    % This calculation appeared in two different places, so I moved it into
    % a separate function. 
    % Input parameters have the same name as original variables in calling
    % function.
    R  = a(1);
    Di = a(2);
    De = a(3);
    ...
    u_vec = rootsS.';
    l_vec = (u_vec/R).^2;
    B_vec = (2*(R./u_vec).^2)./(u_vec.^2 - 2);
    W_vec = (-2).*Di.^(-2).*e.^(-2).*exp(1).^((-1/2).*Di.*l_vec.*v.^(-1).*(N+2.*(del+DEL).* ...
        v)).*(exp(1).^((1/2).*Di.*del.*l_vec)+(-1).*exp(1).^((1/4).*Di.*N.*l_vec.*v.^(-1)) ...
        ).^4.*Grad.^2.*l_vec.^(-2)+2.*Di.^(-2).*e.^(-2).*exp(1).^((-1/2).*Di.*l_vec.*v.^( ...
        -1).*(N+2.*(2.*e+DEL).*v)).*((-1)+exp(1).^(Di.*e.*l_vec)).^2.*(exp(1).^(Di.* ...
        e.*l_vec)+(-1).*exp(1).^((1/2).*Di.*l_vec.*v.^(-1))).^2.*(1+exp(1).^((1/2).* ...
        Di.*l_vec.*v.^(-1))).^(-2).*((-1).^(1+N)+exp(1).^((1/2).*Di.*N.*l_vec.*v.^(-1))) ...
        .*((-1)+(-1).^N.*exp(1).^((1/2).*Di.*N.*l_vec.*v.^(-1))).*Grad.^2.*l_vec.^(-2)+2.*( ...
        -1).^N.*Di.^(-2).*e.^(-2).*exp(1).^((-1/2).*Di.*l_vec.*v.^(-1).*(N+2.*(e+ ...
        del+DEL).*v)).*((-1)+exp(1).^(Di.*e.*l_vec)).*(exp(1).^(Di.*e.*l_vec)+(-1).*exp(1) ...
        .^((1/2).*Di.*l_vec.*v.^(-1))).*(1+exp(1).^((1/2).*Di.*l_vec.*v.^(-1))).^(-1).*( ...
        exp(1).^((1/2).*Di.*del.*l_vec)+(-1).*exp(1).^((1/4).*Di.*N.*l_vec.*v.^(-1))).^2.* ...
        (1+(-1).^(1+N).*exp(1).^(Di.*del.*l_vec)+(-2).*exp(1).^(Di.*DEL.*l_vec)+exp(1).^( ...
        Di.*del.*l_vec+(1/2).*Di.*N.*l_vec.*v.^(-1))+2.*(-1).^N.*exp(1).^(Di.*DEL.*l_vec+(1/2).* ...
        Di.*N.*l_vec.*v.^(-1))+(-1).^(1+N).*exp(1).^((1/2).*Di.*N.*l_vec.*v.^(-1))).* ...
        Grad.^2.*l_vec.^(-2)+(-4).*(-1).^N.*Di.^(-2).*e.^(-2).*exp(1).^((-1/2).*Di.* ...
        l_vec.*v.^(-1).*(1+N+4.*e.*v)).*((-1)+exp(1).^(Di.*e.*l_vec)).^2.*(exp(1).^( ...
        Di.*e.*l_vec)+(-1).*exp(1).^((1/2).*Di.*l_vec.*v.^(-1))).^2.*(1+exp(1).^((1/2) ...
        .*Di.*l_vec.*v.^(-1))).^(-2).*Grad.^2.*(exp(1).^((1/2).*Di.*l_vec.*v.^(-1))+(-1) ...
        .^N.*exp(1).^((1/2).*Di.*(1+N).*l_vec.*v.^(-1)).*((-1)+N)+(-1).^N.*exp(1).^( ...
        (1/2).*Di.*N.*l_vec.*v.^(-1)).*N).*l_vec.^(-2)+(-2/3).*Di.^(-2).*e.^(-2).*exp( ...
        1).^((-1/2).*Di.*l_vec.*(2.*e+v.^(-1))).*Grad.^2.*N.*l_vec.^(-2).*v.^(-1).*((-6).* ...
        ((-1)+exp(1).^(Di.*e.*l_vec)).*((-1).*exp(1).^(Di.*e.*l_vec)+exp(1).^(2.*Di.* ...
        e.*l_vec)+2.*exp(1).^((1/2).*Di.*l_vec.*v.^(-1))).*v+12.*Di.*e.*exp(1).^((1/2) ...
        .*Di.*l_vec.*(2.*e+v.^(-1))).*l_vec.*v+Di.^3.*e.^2.*exp(1).^((1/2).*Di.*l_vec.*( ...
        2.*e+v.^(-1))).*l_vec.^3.*((-3)+8.*e.*v))+(1/96).*Di.^(-2).*e.^(-2).*l_vec.^( ...
        -2).*v.^(-3).*((-192).*((-1)+exp(1).^((1/4).*Di.*l_vec.*v.^(-1).*(N+(-2).* ...
        del.*v))).*(e+Grad.^2).*v.^3+(-1).*Di.^3.*l_vec.^3.*(N+(-2).*del.*v).^2.*((-6).* ...
        e.^2.*v+Grad.^2.*(N+(-2).*del.*v)+e.*(N+4.*del.*v))+(-6).*Di.^2.*l_vec.^2.*v.*(N+ ...
        (-2).*del.*v).*((-8).*e.^2.*v+Grad.^2.*(N+(-2).*del.*v)+e.*(N+6.*del.*v))+24.* ...
        exp(1).^((-1).*Di.*del.*l_vec).*v.*((-4).*exp(1).^((1/4).*Di.*N.*l_vec.*v.^(-1)).* ...
        Grad.*v+exp(1).^((1/2).*Di.*del.*l_vec).*Grad.*(4.*v+Di.*l_vec.*(N+(-2).*del.*v))).^2+2.*( ...
        -1).^(2.*N).*exp(1).^((-1/2).*Di.*del.*l_vec).*Grad.^2.*(48.*exp(1).^((1/4).*Di.* ...
        N.*l_vec.*v.^(-1)).*v.^2.*((-4).*v+Di.*l_vec.*(N+(-2).*del.*v))+exp(1).^((1/2).* ...
        Di.*del.*l_vec).*(192.*v.^3+(-6).*Di.^2.*l_vec.^2.*v.*(N+(-2).*del.*v).^2+(-1).* ...
        Di.^3.*l_vec.^3.*(N+(-2).*del.*v).^3))+(-48).*Di.*exp(1).^((-1/2).*Di.*del.*l_vec).* ...
        l_vec.*v.^2.*((-4).*e.^2.*(exp(1).^((1/2).*Di.*del.*l_vec)+(-1).*exp(1).^((1/4).* ...
        Di.*N.*l_vec.*v.^(-1))).*v+(-1).*exp(1).^((1/4).*Di.*N.*l_vec.*v.^(-1)).*Grad.^2.*( ...
    ...
    icsig_vec = exp((-0.5.*gamma.^2./Di.^2).*sumTerm_vec);
